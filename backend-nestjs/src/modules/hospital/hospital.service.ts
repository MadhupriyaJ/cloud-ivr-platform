...
export class HospitalService {
  ...
  constructor(
    ...
    private readonly domainIntentRepository: Repository<DomainIntentEntity>,
    @InjectRepository(DomainRuleEntity)
    private readonly domainRuleRepository: Repository<DomainRuleEntity>,
  ...
  async bootstrap(payload: BootstrapHospitalDto): Promise<any> {
    ...
    if (!domain) {
      domain = await this.domainRepository.save(
  ...
  async listDoctors(departmentId?: string): Promise<any[]> {
    const query = this.doctorRepository
      .createQueryBuilder('doctor')
      .leftJoin(DepartmentEntity, 'department', 'department.DepartmentId = doctor.DepartmentId')
      .select([
  ...
  async listAvailableDoctors(departmentId?: string, date?: string): Promise<any[]> {
    await this.ensureSchedulesForActiveDoctors(departmentId);

    const query = this.doctorRepository
      .createQueryBuilder('doctor')
      .innerJoin(DoctorScheduleEntity, 'schedule', 'schedule.DoctorId = doctor.DoctorId')
      .innerJoin(DepartmentEntity, 'department', 'department.DepartmentId = doctor.DepartmentId')
      .select([
        'doctor.doctorId AS doctorId',
        'doctor.doctorName AS doctorName',
        'doctor.specialization AS specialization',
  ...
  async getDoctorSlots(doctorId: string, date?: string): Promise<DoctorScheduleEntity[]> {
  ...
  async createPatient(payload: CreatePatientDto): Promise<PatientEntity> {
    ...
    if (existing) {
      const nextPayload: Partial<CreatePatientDto> = {
        ...payload,
      };
  ...
  async lookupPatient(patientCode?: string, phoneNumber?: string): Promise<PatientEntity | null> {
  ...
  async createAppointment(payload: CreateAppointmentDto): Promise<any> {
    ...
    if (!patient) {
      if (!payload.patientName || !payload.phoneNumber) {
        throw new NotFoundException(
          'Patient not found. Provide patientId, patientCode, or patientName with phoneNumber.',
        );
      }
    ...
    if (existingAppointment) {
      throw new ConflictException(
        `An active appointment already exists for this patient with ${existingAppointment.doctorName} on ${existingAppointment.appointmentDate} at ${existingAppointment.appointmentTime}.`,
      );
    }

    const doctor = await this.doctorRepository.findOne({ where: { doctorId: payload.doctorId } });
    if (!doctor) {
      throw new NotFoundException(`Doctor '${payload.doctorId}' not found.`);
    }

    const schedule = await this.findScheduleByDoctorDateTime(
      payload.doctorId,
    ...
    if (!schedule || schedule.availableSlots <= 0) {
      throw new NotFoundException('No available slot found for the requested doctor, date, and time.');
    }
  ...
  async verifyAppointment(patientCode?: string, phoneNumber?: string): Promise<any | null> {
  ...
  async listAppointments(filters: {
    ...
    const query = this.appointmentRepository
      ...
      .innerJoin(PatientEntity, 'patient', 'patient.PatientId = appointment.PatientId')
      .innerJoin(DoctorEntity, 'doctor', 'doctor.DoctorId = appointment.DoctorId')
      .innerJoin(DepartmentEntity, 'department', 'department.DepartmentId = appointment.DepartmentId')
      .select([
  ...
  async rescheduleAppointment(appointmentId: string, payload: RescheduleAppointmentDto): Promise<any> {
    const appointmentTime = this.normalizeTime(payload.appointmentTime);
    ...
    const nextSchedule = await this.findScheduleByDoctorDateTime(
    ...
    if (!nextSchedule || nextSchedule.availableSlots <= 0) {
      throw new NotFoundException('Requested reschedule slot is unavailable.');
    }

    const previousSchedule = await this.findScheduleByDoctorDateTime(
      appointment.doctorId,
  ...
  async getBilling(patientCode?: string, phoneNumber?: string): Promise<BillingEntity[]> {
    const patient = await this.lookupPatient(patientCode, phoneNumber);
    if (!patient) {
      return [];
    }
  ...
  async getLabReports(patientCode?: string, phoneNumber?: string): Promise<LabReportEntity[]> {
    const patient = await this.lookupPatient(patientCode, phoneNumber);
    if (!patient) {
      return [];
    }
  ...
  async runIvrStep(payload: HospitalIvrRequestDto): Promise<any> {
      ...
      if (nextIntent === 'book-appointment') {
        ...
        return {
          ...
          prompt: await this.composeHospitalReply({
            ...
            fallback:
              'Please tell me the department name. For example cardiology, neurology, orthopedics, or pediatrics.',
            utterance,
    ...
    if (step === 'book-choose-doctor') {
      if (payload.departmentId && this.isDoctorListRequest(utterance)) {
        const doctors = await this.listAvailableDoctors(
          payload.departmentId,
          appointmentDate || this.currentLocalDate(),
        );
    ...
    if (step === 'book-choose-slot') {
      ...
      if (!appointmentTime) {
        const openSlots = (await this.getDoctorSlots(payload.doctorId, appointmentDate || this.currentLocalDate())).filter(
          (item) => item.availableSlots > 0,
        );
        return {
          nextStep: 'book-choose-slot',
          prompt: await this.composeHospitalReply({
  ...
  private resolveIntent(currentIntent?: string, utterance?: string): string | undefined {
    ...
    if (text.includes('bill')) return 'billing';
    if (text.includes('lab') || text.includes('report')) return 'lab-report';
    if (
  ...
  private isDoctorListRequest(utterance?: string): boolean {
    const text = (utterance ?? '').toLowerCase();
    if (!text) return false;

    return (
      (text.includes('doctor') && text.includes('available')) ||
  ...
  private isDoctorDirectoryRequest(utterance?: string): boolean {
    const text = (utterance ?? '').toLowerCase();
    if (!text) return false;

    return (
      (text.includes('doctor') && text.includes('department')) ||
  ...
  private buildUnsupportedPrompt(utterance?: string, unsupportedCount = 0): string {
  ...
  private async composeHospitalReply(input: {
    goal: string;
    fallback: string;
    utterance?: string;
    ...
    const deployment = (
      process.env.AZURE_OPENAI_ORCHESTRATION_DEPLOYMENT ||
      process.env.AZURE_OPENAI_DEPLOYMENT ||
      ''
    ).trim();
    const apiKey = (process.env.AZURE_OPENAI_API_KEY || '').trim();
    const apiVersion = (
      process.env.AZURE_OPENAI_ORCHESTRATION_API_VERSION ||
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.warn(
          `Hospital AI reply fallback used because Azure OpenAI returned ${response.status}: ${errorText.slice(0, 300)}`,
        );
        return input.fallback;
      }
      ...
      this.logger.warn(
        `Hospital AI reply fallback used because Azure OpenAI request failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      return input.fallback;
    }
  }

  private async getHospitalAiSystemPrompt(): Promise<string> {
    ...
    return (
      activeSystemPrompt?.templateText?.trim() ||
  ...
  private extractPatientCode(text?: string): string | undefined {
  ...
  private isLikelyPatientNameUtterance(text?: string, knownPatientName?: string): boolean {
    const value = (text ?? '').trim();
    if (!value) {
      return false;
    }
    ...
    return (
      normalizedValue === normalizedExtracted ||
  ...
  private cleanPatientName(value?: string): string | undefined {
    if (!value) return undefined;
  ...
  private shouldReplacePatientName(currentName?: string | null, nextName?: string | null): boolean {
  ...
  private isWeakPatientName(value: string): boolean {
    const normalized = value.toLowerCase().trim();
    return ['hello', 'hey', 'hi', 'name', 'patient', 'unknown'].includes(normalized);
  }

  private buildAppointmentConfirmation(appointment: any): string {
  ...
  private buildExistingAppointmentSummary(appointment: any): string {
    const patientName = this.cleanPatientName(appointment.patientName) || appointment.patientName || 'Unknown patient';
    const doctorName = appointment.doctorName || 'Unknown doctor';
    const departmentName = appointment.departmentName || 'Unknown department';
    const appointmentDate = this.formatDateForSpeech(appointment.appointmentDate);
  ...
  private buildCancelledAppointmentSummary(appointment: any): string {
    const doctorName = appointment.doctorName || 'Unknown doctor';
    const departmentName = appointment.departmentName || 'Unknown department';
    const appointmentDate = this.formatDateForSpeech(appointment.appointmentDate);
    const appointmentTime = this.formatTimeForSpeech(appointment.appointmentTime);
  ...
  private buildDoctorDirectorySummary(doctors: any[]): string {
    if (!doctors.length) {
  ...
  private async findActiveAppointmentForPatient(patientId: string): Promise<any | null> {
  ...
  private formatDateForSpeech(value?: unknown): string {
    if (!value) {
  ...
  private formatTimeForSpeech(value?: unknown): string {
    if (!value) {
      return 'not available';
    }
  ...
  private tryParseDateValue(raw: string): Date | undefined {
    if (!raw) {
      return undefined;
    }
  ...
  private resolveDate(currentDate?: string, utterance?: string): string | undefined {
    if (currentDate?.trim()) {
    ...
    if (text.includes('today')) return this.currentLocalDate();
    if (text.includes('tomorrow')) return this.offsetLocalDate(1);
    const match = text.match(/\b(20\d{2}-\d{2}-\d{2})\b/);
    return match?.[1];
  }

  private resolveTime(currentTime?: string, utterance?: string): string | undefined {
    if (currentTime?.trim()) {
  ...
  private offsetLocalDate(offset: number): string {
  ...
  private async matchDepartmentId(utterance?: string): Promise<string | undefined> {
  ...
  private normalizeTime(value: string): string {
    ...
    throw new NotFoundException('Invalid appointment time format. Use HH:mm or HH:mm:ss.');
  }

  private async findScheduleByDoctorDateTime(
    doctorId: string,
    scheduleDate: string,
    startTime: string,
  ...
  private async seedHospitalMetadata(domainId: string, organizationName: string): Promise<void> {
    const intents = [
    ...
    for (const [intentCode, intentLabel, description] of intents) {
      ...
      if (!exists) {
        await this.domainIntentRepository.save(
          this.domainIntentRepository.create({
            domainId,
            intentCode,
            intentLabel,
            description,
    ...
    for (const [ruleType, ruleText] of rules) {
      ...
      if (!exists) {
        await this.domainRuleRepository.save(
          this.domainRuleRepository.create({
            domainId,
            ruleType,
            ruleText,
            priority: 100,
    ...
    for (const [promptType, templateText] of prompts) {
      ...
      if (!exists) {
        await this.promptTemplateRepository.save(
    ...
    for (const [toolName, description, handlerName, schemaJson] of tools) {
      ...
      if (!exists) {
        await this.toolDefinitionRepository.save(
          this.toolDefinitionRepository.create({
            domainId,
            toolName,
            description,
            handlerName,
  ...
  private async seedDepartments(domainId: string): Promise<DepartmentEntity[]> {
    const seeds = [
    ...
    const created: DepartmentEntity[] = [];
  ...
  private async seedDoctors(domainId: string, departments: DepartmentEntity[]): Promise<DoctorEntity[]> {
  ...
  private async seedSchedules(doctors: DoctorEntity[]): Promise<DoctorScheduleEntity[]> {
  ...
  private async ensureSchedulesForActiveDoctors(departmentId?: string): Promise<void> {
  ...
  private async ensureSchedulesForDoctor(doctorId: string): Promise<void> {
